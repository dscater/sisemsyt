...
class HistorialAccionService
{
    ...
    public function registrarAccion(string $modulo, string $accion, string $descripcion = "", Model $modelo, Model $modelo_update = null, array $relaciones = []): void
    {
        ...
        if (!empty($relaciones)) {
            $datos_original = $modelo->loadMissing($relaciones)->makeHidden($modelo->getMutatedAttributes())->toArray();
        } else {
            $datos_original = $modelo->makeHidden($modelo->getMutatedAttributes())->toArray();
        }
        $datos = [
            "user_id" => $user_id,
            "accion" => $accion,
        ...
        if ($modelo_update) {
            ...
            if ($existe_cambios && $modelo_update) {
                // actualizacion
                if (!empty($relaciones)) {
                    $datos["datos_nuevo"] = $modelo_update->loadMissing($relaciones)->makeHidden($modelo_update->getMutatedAttributes())->toArray();
                } else {
                    $datos["datos_nuevo"] = $modelo_update->makeHidden($modelo_update->getMutatedAttributes())->toArray();
                }
